...
# skriver ut hvilken ukedag den faller pÃ¥.
...
# Gitt et mÃ¥nedsnavn, finn mÃ¥nedens nummer.
# Svar 0 om det ikke er et lovlig mÃ¥nedsnavn.
def finn_maaned (m_id):
   for m_ix in range(1,len(m_navn)):
      if m_navn[m_ix] == m_id: return m_ix
   return 0

# Les et mÃ¥nedsnavn; fortsett til lovlig navn.
def les_maaned ():
   while True:
...
# Er angitte Ã¥r et skuddÃ¥r?
def er_skuddaar (aa):
   return aa%4==0 and aa%100 or aa%400==0

# Beregn antall dager i en gitt mÃ¥ned i et gitt Ã¥r.
...
def finn_dag_nr (aa, m, d):
   ...
   # Tell dagene i Ã¥rene fÃ¸r angitte dato:
   for aax in range(1900,aa):
      d_nr = d_nr + 365;
      if er_skuddaar(aax): d_nr = d_nr + 1

   # Tell dagene i mÃ¥nedene fÃ¸r:
   for mx in range(1,m):
